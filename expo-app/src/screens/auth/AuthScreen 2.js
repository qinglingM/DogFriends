...
export default function AuthScreen() {
  ...
  const handleSubmit = async () => {
    if (!isConfigured) {
      setMessage({
        type: 'error',
        text: '请先配置 EXPO_PUBLIC_SUPABASE_URL 和 EXPO_PUBLIC_SUPABASE_ANON_KEY',
      });
      return;
    }
    if (!normalizedPhone) {
    ...
    >
      ...
      >
        ...
        <View style={styles.card}>
          ...
          {!isConfigured && (
            <View style={styles.configNotice}>
              ...
              <Text style={styles.configText}>
                当前为接入预览。配置 Supabase 环境变量后可发送真实验证码。
              </Text>
            </View>
          )}
          ...
          {mode !== 'password' && (
            <View style={styles.codeRow}>
              ...
              <TouchableOpacity
                ...
                disabled={sending || countdown > 0 || !isConfigured}
                style={[
                  styles.sendButton,
                  (sending || countdown > 0 || !isConfigured) && styles.disabled,
                ]}
              >
                {sending ? (
          ...
          <TouchableOpacity
            style={[styles.submit, (busy || !isConfigured) && styles.disabled]}
...
const styles = StyleSheet.create({
  ...
  configNotice: {
    ...
    gap: spacing.sm,
    padding: spacing.md,
    backgroundColor: colors.tipOrange,
    borderRadius: spacing.radiusMd,
  },
  configText: { ...typography.caption, color: colors.textMain, flex: 1 },
  field: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.sm,
